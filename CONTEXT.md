...
**Tiers 1 a 10 completos y verificados.** El 7 se abrió el 2026-07-27 con una revisión completa del
...
niega si el `HEAD` no es el del último *dry run*. **No hay CI ni la
...
| 2026-08-23 | **El tema se declara también con `color-scheme`, no solo con variables** | La lista que despliega un `select` la pinta WebView2, y para elegir sus colores mira `color-scheme` — **no las variables del tema**. Sin declararlo el navegador supone «claro» siempre, así que en el tema oscuro la lista salía con fondo blanco y encima el texto heredado de `--foreground`, que ahí es casi blanco: ilegible. No se había visto porque **solo pasa con la lista desplegada**, que es cuando ya no sale en una captura de la ventana. Va en `:root` y en `.dark` junto a las variables, para que valga igual a las barras de scroll y al resto de controles nativos |
| 2026-08-23 | **El relleno del `select` y el ancho de su columna se tocan juntos** | La flecha la dibuja el navegador pegada al borde interior del relleno, así que despegarla del borde cuesta relleno y el relleno sale del texto. Subirlo de 8 a 12 px sin tocar la columna devolvía el corte a media palabra. Medido en el motor: el texto ocupa 128,5 px y la flecha 21, así que la columna pasó de 192 a **200** — la cuenta exacta daba 196 y **medio pixel de margen es no tener ninguno** en cuanto cambie la fuente o el zoom |
