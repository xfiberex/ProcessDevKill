...
| 2026-09-25 | **La ventana admite zoom (Ctrl y +, Ctrl y 0)** | Tier 11, E. `zoomHotkeysEnabled` en `tauri.conf.json`, probado con teclas reales. Con zoom el ancho útil baja: al 125 % en la ventana mínima caben 497 px de tabla, y con `table-fixed` la columna del nombre se quedaba en **0 px**. **Las tablas de ancho fijo llevan un mínimo** —procesos 620, servicios 660: las columnas fijas más unos 100 para el nombre— y hacen scroll horizontal. Al 110 % todo cabe a cualquier tamaño |
| 2026-09-25 | **El Historial se agrupa por tandas exactas, no por cercanía** | Tier 11, E. `kill_and_record` da la misma marca y el mismo origen a todo un lote, así que dos entradas seguidas con los dos iguales son una acción. Sin ventana de tiempo: dos Kill en el mismo segundo siguen siendo dos. **Si algún día cada cierre de un lote lleva su propia hora, esto deja de agrupar**, y la prueba de `agruparEnTandas` lo dirá. Hora relativa con `Intl.RelativeTimeFormat` en el idioma de la app y días contados por calendario |
| 2026-09-25 | **Los avisos se quedan abajo a la derecha, y la tabla deja sitio debajo** | Tier 11, E. Medido: tapan la columna Kill de la última fila visible mientras duran. Moverlos arriba tapa Refrescar y Nuke All, y abajo al centro, la barra de la selección. Lo que se hizo es darle al cuerpo de Procesos 80 px de hueco al final, para que cualquier fila pueda subir por encima |
| 2026-09-25 | **Refrescar es un icono mientras hay auto-refresco** | Tier 11, E. Con la lista refrescándose sola cada 2 s, el botón con texto competía con Nuke All por algo que casi no hace falta. En «Off» vuelve con su texto, que es cuando es la única forma de ver datos nuevos |
| 2026-09-24 | **Cada vista pinta su cabecera y su cuerpo con scroll** | Tier 11, D1. `ViewHeader` (título, una línea opcional, acciones) va **fuera del scroll** y `ViewBody` es lo único que se desplaza. El contenedor con scroll salió de App para que la cabecera se quede fija. Procesos la pinta en App, donde vive su estado. **Una vista nueva usa las dos.** Si su cuerpo no tiene nada enfocable, `ViewBody` lleva `label` para entrar en el tabulador: el Historial se quedó sin forma de desplazarse con teclado al subir «Vaciar» a la cabecera |
| 2026-09-24 | **«Cerrar» es el único verbo en español; Kill y Nuke, en inglés** | Tier 11, D3. Había cinco: Kill, Matar, Nuke, Cerrar y Terminar. «Matar» y «Terminar» pasan a «cerrar», que era el que ya usaban la bandeja, las notificaciones y los diálogos; en inglés, «close». **Nuke All con filtro o búsqueda se llama «Nuke filtrados»**: cerraba la lista filtrada y solo lo decía el diálogo |
| 2026-09-24 | **El Kill de cada fila es neutro; el rojo lleno, solo para Nuke All** | Tier 11, D5. **Revisa la decisión del 2026-07-24**, que pedía rojo sólido para «la acción destructiva principal»: esa es Nuke All, no cada Kill, y veinte botones rojos competían con los puertos, que es lo que se viene a mirar. El Kill va gris y se tiñe de rojo con la fila bajo el puntero o con el foco dentro, justo cuando se va a pulsar. Kill sigue sin confirmación, como decidió A5 |
| 2026-09-24 | **La barra de CPU tiene suelo: un núcleo entero** | Tier 11, D6. **Precisa la decisión del 2026-07-23**: las barras se siguen escalando al mayor de la lista, pero la de CPU no baja de `100 / hardwareConcurrency`. En reposo, un proceso al 2,5 % salía con la barra llena. Un núcleo y no una cifra fija porque la CPU de cada proceso va sobre el equipo entero. La RAM se queda sin suelo, que es para lo que se razonó aquella decisión |
