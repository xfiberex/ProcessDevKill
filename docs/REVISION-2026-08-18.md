...
- [x] **[T3-10] El contador de la cabecera es un número sin nombre** — hecho el 2026-08-18
  ...
  - ⚠️ **Anotado el 2026-09-25 (Tier 11, F2): no llegó a cumplirse.** Un `aria-label` sobre un
    `<span>` sin rol no nombra nada: axe lo marcaba y los lectores lo ignoraban, así que se seguía
    anunciando «12» a secas. Lo arregló la B6 del Tier 11 el 2026-09-24: la frase va como texto
    `sr-only` dentro de la región viva, y el número visible, `aria-hidden`.
  - **Esfuerzo:** bajo
  - **Depende de:** ninguna
